...
def train(model, X, Y, A1, A2, optimizer):
...
def train_loop(model, train_dataset, epochs, opt=None):
    ...
    for epoch in range(epochs):
        ...
        for X, Y, A1, A2 in train_dataset:
            ...
            r = train(model, X, Y, A1, A2, optimizer)
            if r:
            ...
            clas_acc += baseline_metrics.accuracy(Y, tf.math.round(Y_hat))

        clas_loss = model.clas_loss
        clas_acc = clas_acc / dataset_size

        print("> {} | {} | {}".format(epoch + 1, clas_loss, clas_acc))
